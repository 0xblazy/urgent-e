...
export default class SwitchLanguageButton extends React.Component{
    ...
    constructor(props) {
        ...
        this.state = {
            current_language : this.props.language,
            languages : ["en","fr"]
        };
    }
    ...
    current_language_label() {
        return this.state.current_language.toUpperCase();
    }
    ...
    render() {
        return (
            <button className="SwitchLanguageButton" onClick={(e) => this.on_click()}>
                <span className="active">{this.current_language_label()}</span>
                <span>{this.not_current_language_label()}</span>
            </button>
        );
    }
}